...
# Data for a company employee roster
...
Daphne Blake,B5678,234-56-7890,(555)222-3333,Sales
...
Velma Dinkley,B1357,678-90-1234,(555)666-7777,R&D
...
Dick Dastardly,B2222,012-34-5678,(555)000-1111,R&D
Penelope Pitstop,C3333,112-23-3445,(555)121-2323,Operations
...
# List the Legal department (which has no employees)
employees.by.primary_dept["Legal"]("Legal").present()

print("\nLookup by name")
for name in "Snagglepuss/Quick Draw McGraw/Secret Squirrel".split("/"):
    employee = employees.by.name[name]
    print(name, employee.primary_phone, employee.employee_id)

print("\nLookup by primary phone number")
for phone_num in "(555)555-6666 (555)444-5555 (555)777-8888".split():
    employee = employees.by.primary_phone[phone_num]
    print(phone_num, employee.name, employee.employee_id)

print("\nLookup by employee id")
for emp_id in "A7890 D3456 C2468".split():
    employee = employees.by.employee_id[emp_id]
    print(emp_id, employee.name, employee.primary_phone)

# Unique indexes raise exception when inserting a duplicate
print("\nTry inserting a record with a duplicate name")
try:
    employees.insert(
        {
        "name": "Dick Dastardly",
...
# By default, unique indexes raise exception when inserting an entry with a null key
print("\nTry inserting a record with a null name")
try:
    employees.insert(
        {
        "name": None,
